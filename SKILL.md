...
| 用户反馈分析   | 上传用户反馈数据（Excel/CSV/文本），自动分类统计并提取高频模式，输出反馈洞察报告和产品改进建议。 |
| 用户故事拆解   | 从Epic或大需求中拆解出独立可交付的User Story，确保符合INVEST原则，含验收标准和Story Point估算。  |
| 竞品分析       | 输入竞品列表，输出功能对比矩阵、SWOT、五力分析和差异化策略。                                     |
| 路线图更新     | 汇总迭代状态、评估里程碑进度、记录优先级变更，输出路线图状态总览和未来规划建议。                 |
| 需求优先级排序 | 输入需求列表，用RICE/ICE/MoSCoW/Kano模型辅助排序，输出透明可追溯的排序结果和Sprint规划建议。     |
...
| 协同技能                        | 典型场景                              |
| ------------------------------- | ------------------------------------- |
| `marketing`(市场营销)           | PRD通过后调用市场营销生成配套营销文案 |
| `consulting-delivery`(咨询交付) | PRD和竞品分析可作为咨询方案框架的输入 |
